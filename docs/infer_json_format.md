...
* `templates`: An optional list of task-level structural templates for protein chains.
...
* `templatesPath`: The path to a precomputed or structural template file for this
  protein chain. Supported formats include `.a3m` (e.g., generated by
  `hmmsearch`), `.hhr`, `.json`, `.cif`, `.mmcif`, and `.pdb`.
...
#### templates
Task-level `templates` lets one structural template file provide templates for
one or more protein chains. This is useful for multichain templates because the
mapping can be specified once instead of creating per-chain template sidecars.
Template features are used only when inference is run with `--use_template true`.
...
* `cif` or `pdb`: Path to a `.cif`, `.mmcif`, or `.pdb` structural template file. Use exactly one.
* `chain_id`: Optional target protein chain ID or list of target chain IDs. If omitted,
  all target protein chains are considered.
* `template_id`: Optional template protein chain ID or list of template chain IDs. If
  omitted, Protenix assigns template chains by sequence alignment.
* `queryIndices` and `templateIndices`: Optional explicit 0-based residue mapping.
  These fields are supported only when a template entry maps one target chain to one
  template chain.

If chain assignment is ambiguous, provide both `chain_id` and `template_id`.
Protein chains covered by task-level `templates` are not sent through automatic
template search during preprocessing.
